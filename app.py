...
def preprocess_input(input_data):
    ...
    geder_map = {0:'Male', 1:'Female'}
    ethinicity_map = {0: 'Moderate', 1: 'Good', 2: 'Excellent'}
    ...
    # Apply mappings
    try:
        input_data['StudentID'] = input_data.get('StudentID', '')
        input_data['Age'] = input_data.get('Age', '')
        input_data['Gender'] = geder_map.get(input_data.get('Gender', ''),0)
        input_data['Ethnicity'] = ethinicity_map.get(input_data.get('Ethnicity', ''),0)
        ...
        input_data['Music'] = yes_no_mapping.get(input_data.get('Music', 0), 0)
        # Convert to DataFrame
        input_df = pd.DataFrame([input_data])

        # Combine categorical features into final dataframe
        final_df = input_df[['StudentID','Age','Gender','Ethnicity','StudyTimeWeekly', 'Absences', 'Tutoring', 'ParentalSupport',
    ...
    if uploaded_file is not None:
        try:
            ...
            for item in process_dict:
                ...
                processed_df_list.append(process_df)  # making to a list after mapping
            my_bar.empty()
            ...
            final_df['Predicted Grade'] = predictions
...
if st.session_state["authenticated"]:
    main_app_model()  # Call the main app function if authenticated
else:
